    ...
    public void _draw() {
        ...
        if (isPreparing) {
            ...
            for (int i = 0; i < laserCount; i++) {
                ...
                drawLine(localCenter, endLocal, warnColor, 3.0f, false);
                // Vẽ một vòng tròn nhỏ cảnh báo ở cuối tia laser
                drawCircle(endLocal, 12.0f, warnColor);
            }
        } else if (isBeaming) {
            ...
            for (int i = 0; i < laserCount; i++) {
                ...
                // Vẽ tia laser ngoài (Độ dày = 18.0px)
                drawLine(localCenter, endLocal, laserOuterColor, 18.0f, false);
                // Vẽ lõi laser trong (Độ dày = 8.0px)
                drawLine(localCenter, endLocal, laserInnerColor, 8.0f, false);
                // Vẽ tâm sáng tại điểm cuối
                drawCircle(endLocal, 22.0f, laserOuterColor);
                drawCircle(endLocal, 12.0f, laserInnerColor);
            }
        }
        ...
        for (LightningStrike strike : activeStrikes) {
            ...
            if (!strike.struck) {
                ...
                drawArc(localPos, lightningRadius * shrinkScale, 0.0f, (float) (Math.PI * 2.0), 32, ringColor, 5.0f, false);
                ...
                for (int s = 1; s <= segments; s++) {
                    ...
                    // Vẽ tia sét phát sáng ngoài (dày 16px, màu xanh lam)
                    drawLine(currentPoint, nextPoint, boltOuter, 16.0f, false);
                    // Vẽ lõi sét trong (dày 6px, màu trắng)
                    drawLine(currentPoint, nextPoint, boltInner, 6.0f, false);

                    currentPoint = nextPoint;
                }

                // Vẽ vụ nổ sáng tại tâm điểm sét đánh trúng đất
                drawCircle(localPos, 35.0f, boltOuter);
                drawCircle(localPos, 18.0f, boltInner);
            }
        }
    }
}
